        ...
        <div class="x_panel pw">
            <div class="x_title">
                <div class="search-form-style-1">
                    @include('backend.pages.search-form')
                        ...
                        <tbody>
                            @foreach($pages as $page)
                                @if($page->page_type == 'page')
                                    <tr id="item-{{$page->id}}">
                                        <td>
                                            @if($page->route == 'blog-page')
                                                blog
                                            @else
                                                {{strip_tags($page->route)}}
                                            @endif
                                        </td>
                                        <td>{{strip_tags($page->title)}}</td>
                                        <td>{{Str::limit(strip_tags($page->content), 70)}}</td>
                                        <td class="text-center table-active-btn">
                                            <a href="{{admin_url('pages/'.$page->id.'/edit')}}" class="btn btn-small btn-primary">Edit</a> |
